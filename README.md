...
- `match_mode`: Controls how multiple input masks are paired. `union` unions both sides into one product, `repeat_mask1` repeats one shared `mask1` reference for every `mask2` product, and `pair_by_index` pairs individual `mask1` and `mask2` products by input order. Default is `union`.
